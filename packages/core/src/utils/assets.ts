...
export function mergeAssets(assets: Assets, incrementalAssets: Assets): Assets {
...
function mergeAssetsComponentList(assets: Assets, incrementalAssets: Assets, listName: keyof Assets): void {
      ...
      if (mods.length > 0) {
        mods.map((item, index) => {
          ...
          window[libraryKeyList[index]] =
            exportMode === 'functionCall' && (exportSourceLibrary == null || exportSourceLibrary === library)
              ? item()
              : item
          return item
        })
